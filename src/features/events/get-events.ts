...
export type GetEventsResult =
    { success: true; events: Event[] } | { success: false; error: string }
...
export async function getEvents(
    ...
    try {
        ...
        if (startDate || endDate) {
            where.date = {}
            if (startDate)
                (where.date as Record<string, Date>).gte = startOfDay(startDate)
            if (endDate)
                (where.date as Record<string, Date>).lte = startOfDay(endDate)
        }
    ...
    try {
        ...
        for (const event of regularEvents) {
            if (event.recurrenceId) {
                const existing =
                    existingByTemplate.get(event.recurrenceId) || new Set()
