...
export function buildCashflowProjection(
	...
	const confirmedBeforeStart = events.filter(
		(e) =>
			e.status === "CONFIRMED" &&
			startOfDay(e.date).getTime() < rangeStart.getTime(),
	)
 ...
 */
export function findCriticalEvents(
...
export function simulatePriorityScenarios(
	...
	if (originalProjection.negativeDays > 0) {
		if (wouldRecover && postponableEvents.length > 0) {
			...
			postponableEvents.length > 0 &&
			projectionWithoutOptional.negativeDays < originalProjection.negativeDays
		) {
			...
			suggestion =
				"Não há gastos opcionais para adiar. Considere revisar suas despesas importantes."
		} else {
			suggestion =
				"Mesmo adiando gastos opcionais, o saldo continuará negativo. Revise suas despesas obrigatórias."
		}
	}
