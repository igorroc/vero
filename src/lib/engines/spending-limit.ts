...
export function calculateDailySpendingLimit(
	...
	const daysUntilHorizon = Math.max(
		1,
		daysBetween(todayStart, horizonStart) - 1,
	)
	...
	if (isNegative) {
		...
		const shortfallFromInvestments =
			effectiveCash - requiredExpenses - plannedInvestments < 0 &&
...
function buildExplanation(breakdown: SpendingLimitBreakdown): string {
	...
	lines.push(`---`)
	lines.push(
		`Available for spending: $${(availableForSpending / 100).toFixed(2)}`,
	)
